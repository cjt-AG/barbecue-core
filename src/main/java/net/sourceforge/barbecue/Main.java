...
public class Main
{
	public static void main(final String[] args)
	{
		...
		try
		{
			...
			switch (encoding) {
			case "ean128": barcode = BarcodeFactory.parseEAN128(barcode_text); break;
			case "code128": barcode = BarcodeFactory.createCode128(barcode_text); break;
			case "upca": barcode = BarcodeFactory.createUPCA(barcode_text); break;
			case "codabar": barcode = BarcodeFactory.createCodabar(barcode_text); break;
			case "ean13": barcode = BarcodeFactory.createEAN13(barcode_text); break;
			default: System.err.println("Unknown encoding: " + encoding);
			}
			...
			{
				try (OutputStreamWriter pw = new OutputStreamWriter(new FileOutputStream(label_outfile))) {
	...
	public static void outputPNG(final Barcode barcode, final OutputStream fos)
	{
		try
		{
	...
	public static void outputSVG(final Barcode barcode,final OutputStream fos)
	{
		try
		{
	...
	public static void outputEPS(final Barcode barcode, final OutputStream fos)
	{
		try
		{
			// We need an output stream to write the image to...
